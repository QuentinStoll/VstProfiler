...
{
    _chooser = std::make_unique<juce::FileChooser>("Sélectionne le WET (Enregistré)", 
        juce::File::getSpecialLocation(juce::File::userDesktopDirectory), "*.wav");
        ...
        [this, &processor](const juce::FileChooser& fc) {
            ...
            if (readerRec != nullptr) {
                ...
                /* LOGIQUE DE DEDUCTION :
                   ...
                   chaque échantillon du WET correspond DIRECTEMENT à un index de la LUT.
                */
                for (int i = 0; i < processor.getLutSize(); ++i) {
                    // On mappe l'index de la LUT sur la position dans le fichier audio
                    float progress = (float)i / (float)(processor.getLutSize() - 1);
